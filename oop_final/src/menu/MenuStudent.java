...
public class MenuStudent extends MenuUser{
	...
	public MenuStudent() {
		super.setMenu("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n" +
					  ...
					  "4.View transcript\n" +
					  "5.View Marks\n" + 
					  "6.View info about Teacher of a specific course\n" + 
	...
	public boolean choosingAction(String act) throws IOException, InterruptedException {
		...
		else if(act.equals("5")) viewMarks();
		else if(act.equals("6")) viewInfoAboutTeacher();
		else if(act.equals("7")) downloadTranscript();
		else if(act.equals("8")) rateTeacher();
		else if(act.equals("9")) viewNews();
		else if(act.equals("10")) orderBook();
		else if(act.equals("11")) returnBook();
...
//		 }
	 ...
	 public void viewTranscript() {
		 ...
		 System.out.println("Your overall GPA: " + student.calculateGPA());
	 ...
	 public void viewMarks() throws NumberFormatException, IOException {
		 if(student.getGrades().size() == 0) {
			 System.out.println("The student does not have any course");
			 return;
		 }
		 System.out.println("\nEnter course code to see course marks: ");
		 String code = Buffer.reader.readLine();
		 for(Course c : student.getGrades().keySet()) {
			 if(c.getCode().equals(code)) {
				 System.out.println("Course: " + c.getName() + ", marks: " + student.getGrades().get(c));
				 break;
			 }
		 }
	 }
	 
	 public void viewInfoAboutTeacher() throws IOException, InterruptedException{
	      Vector<Course> courses = new Vector <Course>();
	      for(Map.Entry<Course, Mark> item : student.getGrades().entrySet()) {
	    	  courses.add(item.getKey());
	      }
	       if(!courses.isEmpty()) {
	        ...
	        for(Course c : courses) {
	           ...
	           System.out.println("\nWhich course teacher's information do you want to see: ");
	           int choice = Integer.parseInt(Buffer.reader.readLine()) - 1;
	           Vector<Teacher> teachers = new Vector<>(Database.getDatabase().getTeachersOfCourse(courses.get(choice)));
	           if (teachers.isEmpty()) {
	               System.out.println("\nNo teachers for this course");
	        ...
	        for(Teacher t : teachers) {
	           ...
	           System.out.print("Choose teacher: ");
	           choice = Integer.parseInt(Buffer.reader.readLine()) - 1;
	           System.out.println(teachers.get(choice));
	       }else System.out.println("You don't register any courses :(");
	   }
}
