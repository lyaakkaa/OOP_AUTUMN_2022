...
public class MenuAdmin extends MenuUser{
	...
	public void viewUsers() {
		...
		for(User u: admin.viewUsers()) {
			System.out.println(index + ")" + u.getClass().getName().substring(8) + ": " + u.getName() + " " + u.getSurname());
			index++;
		}
	}
	...
	public void removeUser() throws IOException, InterruptedException {
		System.out.println("Choose which user do you want to delete: ");
		this.viewUsers();
		int choice = Integer.parseInt(Buffer.reader.readLine()) - 1;
		admin.removeUser((User)admin.viewUsers().get(choice));
		System.out.println("User has been sucessfully deleted!");
	}
	
	public void updateUser() throws IOException, InterruptedException {
		System.out.println("Choose which user do you want to update: ");
		this.viewUsers();
		int choice = Integer.parseInt(Buffer.reader.readLine()) - 1;
		User updUser = admin.viewUsers().get(choice);
