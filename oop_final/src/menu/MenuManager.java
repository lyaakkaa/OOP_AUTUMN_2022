...
public class MenuManager extends MenuUser {
	...
	public void addCourse() throws IOException {
		...
		for(int i = 0; i < numLessons; i++) {
			...
			LessonType lt;
			if(choosenType.equals("1")) lt = LessonType.LECTURE;
			else if(choosenType.equals("2")) lt = LessonType.PRACTICE;
			else if(choosenType.equals("3")) lt = LessonType.LAB;
			else {
	...
	public void viewStudents() throws IOException{
		...
		for(Student s : students) {
			System.out.println(index + ")" + s.getName() + " " + s.getSurname() + " " + s.calculateGPA());
		}
	}
	...
	public void manageNews() throws IOException {
		Vector<News> newss = Database.getDatabase().getNews();
        Buffer.viewNews();
        if(!newss.isEmpty()) {
            ...
            changeNews(newss.get(choice));
        }
	}
	
	public void changeNews(News news) throws IOException {
		System.out.println("What do you wish to change?\n1. Title 2.Text 3.Clear comments");
		switch(Buffer.reader.readLine()) {
			case "1" -> {
	            System.out.print("Previous title: " + news.getTitle() + "\nEnter a new title: ");
	            news.setTitle(Buffer.reader.readLine());
	        }
	        case "2" -> {
	            System.out.print("Previous text: " + news.getTitle() + "\nEnter a new text: ");
	            news.setText(Buffer.reader.readLine());
	        }
	        case "3" -> {
	...
	public void readMessages() {
		...
		for(Message message : Database.getDatabase().getMessages()) {
			if(message.getReceiver() == manager) {
				messages.add(message);
			}
		}
		if(messages.isEmpty()) {
			System.out.println("Unfortunately there are no messages");
			return;
		}
		else {
