...
public class UserFactory {
	...
	public User createNewUser(String uType) throws IOException {
		if(uType == "admin") return createNewAdmin();
		else if(uType == "teacher") return createNewTeacher();
		else if(uType == "student") return createNewStudent();
		else if(uType == "manager") return createNewManager();
		else if(uType == "librarian") return createNewLibrarian();
		return null;
	}
	...
	public Student createNewStudent() throws IOException{
		...
		return new Student(name, surname, password, yearOfStudy, id, degree, faculty, speciality);
	...
	public Manager createNewManager() throws IOException{
		...
		String type = Buffer.reader.readLine();
		if(type == "1") managerType = ManagerType.DEPARTMENTS;
		else if(type == "2") managerType = ManagerType.OR;
